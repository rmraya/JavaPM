...
public class ExportDialog extends Dialog {
	...
	public ExportDialog(Shell parent) {
		...
		shell.setText(Messages.getString("ExportDialog.9")); //$NON-NLS-1$
		...
		fileLabel.setText(Messages.getString("ExportDialog.0")); //$NON-NLS-1$
		...
		browse.setText(Messages.getString("ExportDialog.1")); //$NON-NLS-1$
		browse.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				String[] names = {Messages.getString("ExportDialog.2"),Messages.getString("ExportDialog.3")}; //$NON-NLS-1$ //$NON-NLS-2$
				fd.setFilterExtensions(extensions);
				fd.setFilterNames(names);
				fd.setFilterPath(DirectoryTracker.lastDirectory(Constants.preferences));
				String output = fd.open();
				if (output != null) {
					if (!output.endsWith(".xlf")) { //$NON-NLS-1$
						output = output + ".xlf"; //$NON-NLS-1$
					}
		...
		languageLabel.setText(Messages.getString("ExportDialog.4")); //$NON-NLS-1$
		...
		okButton.setText(Messages.getString("ExportDialog.5")); //$NON-NLS-1$
		...
		okButton.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent arg0) {
				if (fileText.getText().equals("")) { //$NON-NLS-1$
					...
					box.setMessage(Messages.getString("ExportDialog.6")); //$NON-NLS-1$
					box.open();
					return;
				}
				if (langCombo.getText().equals("")) { //$NON-NLS-1$
					MessageBox box = new MessageBox(shell,SWT.ICON_WARNING|SWT.OK);
					box.setMessage(Messages.getString("ExportDialog.7")); //$NON-NLS-1$
					box.open();
					return;
				}
		...
		cancel.setText(Messages.getString("ExportDialog.8")); //$NON-NLS-1$
